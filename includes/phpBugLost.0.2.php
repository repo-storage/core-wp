...
//define('_bl_css_file', '/source/pbl_styles.css');
define('_bl_css_file', '');

// use external js file. Multiple with coma.
//define('_bl_js_file', '/source/pbl_scripts.js');
define('_bl_js_file', '');
...
define('_bl_path', str_replace($_SERVER['DOCUMENT_ROOT'], '/', str_replace('\\',
	'/', __file__)));
...
// shortcuts tags, html markup for top menu
if (_bl_keyboard_shortcuts == true) {
	...
	define('_bl_key_ajax', '<sup>6</sup>');
	define('_bl_key_php', '<sup>7</sup>');
	define('_bl_key_jscss', ' (8)');
	define('_bl_key_opacity', ' (9)');
	define('_bl_key_info', ' (0)');
} else {
	define('_bl_key_logs', '');
	define('_bl_key_sql', '');
	define('_bl_key_vars', '');
...
class _bl {
	...
	public static $errors = false; // used for error alert
	public static $msgs = array();
	public static $msgs_time = array();
	public static $msg_sql = array();
	public static $time_start = _bl_time_start;
	public static $panel_state = 'close';
	public static $panel_active = array(
		"msg" => "bl_debug_panel_active",
		"sql" => "",
		"vars" => "",
		"time" => "",
		"memory" => "",
	...
	if (count($array)) {
		...
		foreach ($array as $k => $v) {
			...
			if (!$error) {
				...
				$var_size = strlen(serialize($v)); // get var size (memory)
 ...
 */
function bl_debug($defined_vars = array(), $only_monitor = false) {
	...
	// end if use as monitor
	//if ($only_monitor) return false;

	// create bookmarklets
	bl_create_bookmarklets();

	/* get vars, functions, classes and constants */
	...
	// Finally, generate and return de HTML code for the debug panel
